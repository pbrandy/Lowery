...
namespace Lowery
{
	public static class TableExtensions
	{
			...
			return await QueuedTask.Run(async () =>
			{
				...
				while (cursor.MoveNext())
				{
					...
					foreach (var prop in propInfo["Related"])
					{
						...
						var genericMethod = typeof(RelationshipExtentions).GetMethod("GetRelated")?.MakeGenericMethod(new Type[] { relatedType });

						if (genericMethod?.Invoke(null, new object[] { relationship, oid, table.GetName() }) is Task task)
						{
							await task.ConfigureAwait(false);
							var resultProperty = task.GetType().GetProperty("Result");
							if (relatedIsEnumerable)
								prop.PropertyInfo.SetValue(newEntity, resultProperty?.GetValue(task) ?? null);
							else
							{
								IEnumerable<object>? result = (IEnumerable<object>?)resultProperty?.GetValue(task);
								if (result != null)
		...
		public static async Task AltUpdate<T>(this Table table, T value)
		{
			List<T> results = new();
			Dictionary<string, List<ExpandedPropertyInfo>> propInfo = Common.SortPropertyInfo(typeof(T));
			...
			await QueuedTask.Run(() => {
				...
				editOperation.Callback(context =>
				{
					...
					while (rowCursor.MoveNext())
					{
						...
						foreach (var prop in propInfo["Standard"])
						{
